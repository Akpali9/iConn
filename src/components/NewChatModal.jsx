          ...
          {mode === 'group' && (
            <div style={{ marginBottom: 12 }}>
              <input
                ...
                style={{ borderRadius: 'var(--r-sm)', padding: '10px 14px', background: 'var(--canvas)', border: '1.5px solid var(--canvas-3)', width: '100%' }}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
                  {sel.map(u => (
                    <div key={u.id} style={{ display:'flex', alignItems:'center', gap:5, background:'var(--canvas)', border:'1.5px solid var(--canvas-3)', borderRadius:99, padding:'3px 10px 3px 5px', fontSize:13 }}>
                      ...
                      <button onClick={() => toggle(u)} style={{ background:'none', border:'none', color:'var(--ink-40)', fontSize:15, lineHeight:1, padding:0, marginLeft:2 }}>×</button>
                    </div>
                  ))}
                </div>
              )}
