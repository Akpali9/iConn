...
function ConvRow({ c, active, onClick, onShowProfile, onDelete }) {
  ...
  return (
    <div className={`conv-item ${active ? 'active' : ''}`} onClick={onClick}>
      <div className="av" style={{ position: 'relative', cursor: isGroup ? 'default' : 'pointer' }} onClick={(e) => { e.stopPropagation(); if (!isGroup && member) onShowProfile(member); }}>
        {isGroup ? (
          c.avatar_url ? (
            <img src={c.avatar_url} alt={name} className="av-img" style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover' }} />
          ) : (
            <GrpAv members={c.members} />
          )
        ) : (
      ...
      <div className="conv-body">
        ...
        <div className="conv-row2"><span className="conv-preview">{isGroup ? `${(c.members?.length||0)+1} members` : online ? 'Online now' : 'Tap to chat'}</span></div>
      </div>
