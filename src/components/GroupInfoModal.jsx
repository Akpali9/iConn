...
export default function GroupInfoModal({ conversation, onClose, onUpdate, currentUserId }) {
  ...
  const confirmDeleteGroup = () => {
    setConfirmDialog({
      isOpen: true,
      title: 'Delete Group',
      message: 'Delete this group permanently? All messages will be lost for everyone. This action cannot be undone.',
      confirmText: 'Delete',
      onConfirm: async () => {
        setConfirmDialog({ ...confirmDialog, isOpen: false })
        setLoading(true)
        ...
        <div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
          ...
          <div className="modal-body">
            ...
            <div style={{ marginTop: 20, display: 'flex', gap: 12, flexDirection: 'column' }}>
              <button className="btn-primary" style={{ background: 'var(--error)' }} onClick={confirmLeaveGroup}>
                <LogOut size={16} style={{ marginRight: 8 }} /> Leave Group
              </button>
              {isAdmin && (
                <button className="btn-primary" style={{ background: 'var(--error)' }} onClick={confirmDeleteGroup}>
